...
def render():
    ...
    with col_nav3:
        if st.button("✏️ Edición", use_container_width=True,
...
def render_catalogo():
    ...
    with col_busq1:
        busqueda = st.text_input("🔍 Buscar", placeholder="Nombre o SKU...")
    
    with col_busq2:
        cat_options = ["Todas"] + [c.get("nombre", "Sin categoría") for c in categorias]
        cat_filtro = st.selectbox("Categoría", cat_options)
    
    with col_busq3:
        estado_options = ["Todos", "Agotado", "Crítico", "Bajo", "Saludable"]
        estado_filtro = st.selectbox("Estado", estado_options)
    ...
    if estado_filtro != "Todos":
        ...
        productos_filtrados = [p for p in productos_filtrados if get_prod_estado(p.get("id")) == estado_filtro]
    ...
    if productos:
        ...
        with col_exp_izq:
            st.download_button("📊 Excel", data=excel_data, file_name="productos.xlsx", use_container_width=True)
        with col_exp_der:
            json_data = df_export.to_json(orient='records', indent=2)
            st.download_button("📄 JSON", data=json_data, file_name="productos.json", use_container_width=True)
    
    # Grid de productos
    if productos_filtrados:
        for prod in productos_filtrados:
            # Obtener stock
            inv = next((i for i in inventarios if i.get("producto_id") == prod.get("id")), None)
            ...
            with st.container():
                ...
                with col_info:
                    if st.button("✏️ Editar", key=f"edit_{prod.get('id')}"):
                        set_editar_producto(prod.get('id'))
                        st.rerun()
...
def render_nuevo():
    ...
    categorias = api_get("/api/v1/categorias")
    proveedores = api_get("/api/v1/proveedores")
    productos_existentes = api_get("/api/v1/productos")
    ...
    with col1:
        ...
        nombre = st.text_input("Nombre *", key=f"new_nombre_{form_version}", placeholder="Ej: Leche Entera")
    ...
    with col_stock1:
        st.markdown("<label style='color: #94a3b8;'>📥 Unidad de ingreso *</label>", unsafe_allow_html=True)
        unidad_ingreso = st.number_input("", min_value=1, value=10, key=f"new_unidad_ingreso_{form_version}", label_visibility="collapsed")
    
    with col_stock2:
        st.markdown("<label style='color: #94a3b8;'>📦 Stock máximo *</label>", unsafe_allow_html=True)
        stock_max = st.number_input("", min_value=0, value=100, key=f"new_stock_max_{form_version}", label_visibility="collapsed")
    ...
    if not campos_ok:
        ...
        st.button("💾 Crear producto", disabled=True, use_container_width=True)
    else:
...
def render_edicion():
    ...
    with col_btn1:
        ...
        if st.button("💾 Guardar cambios", type="primary", use_container_width=True, disabled=not campos_ok):
            if edit_precio < 0.01:
